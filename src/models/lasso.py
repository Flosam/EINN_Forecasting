...
class Lasso:
    ...
    def fit(self, data: pd.DataFrame, target:str):
        ...
        for iter in range(self.max_iter):
            ...
            if np.max(np.abs(betas - betas_old)) < self.tol:
                log_debug(f"Lasso converged at iteration {iter}")
                break

        self.coefs = betas
        non_zero = np.sum(np.abs(self.coefs) > 1e-10)
        log_debug(f"Lasso fit complete: {non_zero}/{p} non-zero coefficients")
    ...
    def forecast(self, alpha: float = 0.05, n_bootstrap: int = 1000) -> pd.DataFrame:
        ...
        idx = build_forecast_index(last_index, [self.horizon])
